...
class core_lib_wechat
{
  ...
  private function set_php_file($filename, $content) {
	...
	*/
	...
	//有超时设定的post，公众号菜单响应，延迟处理用
	function http_post_timeout($url,$strPOST){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
		...
		curl_setopt($oCurl, CURLOPT_TIMEOUT, 1);//curlopt_timeout
		curl_setopt($oCurl, CURLOPT_URL, $url);
		curl_setopt($oCurl, CURLOPT_RETURNTRANSFER, 1 );
		curl_setopt($oCurl, CURLOPT_POST,true);
		curl_setopt($oCurl, CURLOPT_POSTFIELDS,$strPOST);
	...
	//post 微信公众号图片临时素材
	function http_post_img($url,$param){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
		...
		curl_setopt($oCurl, CURLOPT_URL, $url);
		curl_setopt($oCurl, CURLOPT_RETURNTRANSFER, 1 );
		//curl_setopt($oCurl,CURLOPT_BINARYTRANSFER,true);//二进制
		curl_setopt($oCurl, CURLOPT_POST,true);
	...
	public function checkAuth($appid='',$appsecret=''){
		...
		$data = json_decode($this->get_php_file("d:/wwwroot/mcj/weixinServer/access_token.php"));
		if ($data->expire_time <time())
		{
			...
			if ($result)
			{
				...
				{   
					...
					$this->set_php_file("d:/wwwroot/mcj/weixinServer/access_token.php", json_encode($data));
	 ...
	 * 
	 */
	public function addMedia($media_name){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post_img('https://api.weixin.qq.com/cgi-bin/media/upload?access_token='.$this->access_token.'&type=image',$media_name);
		if ($result)
		{
			$json = json_decode($result,true);
			if (isset($json['errcode'])) {
