...
//			if(!$serial)     throw new Exception("模板编号参数不正确！", 1000);
            ...
            $data['touser']  = $openid;
            return D('Weixin')->send_wx_custom_msg($data['content'],$openid);
		}
		return false;
	}
	/**
...
//			)
//		);
        $content = $data['first'].',订单：'.$data['orderNum'].',商品：'.$data['goodsName'];
...
//			)
//		);
        $content = '亲,您的订单创建成功! 订单：'.$data['order_id'].',商品：'.$data['title'];
        return array('content'=>$content);
	}
